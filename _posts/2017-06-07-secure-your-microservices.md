...
title: 'Securing your cloud-native microservice architecture in Spring'
image: /img/microservices/securitylogo.png
...
* [Creating an User Authentication & Authorization Server](#uaa)
* [Enabling Single Sign On with Spring Session & Redis](#sso)
* [Secure Sensitive Data at Rest](#config)
* [Secure your microservice](#microservice)
* [Secure The Inter-service/Inter-process communication](#communication)
...
<div class="row" style="margin: 0 auto 2.5rem auto; width: 100%;">
  <div class="col-md-offset-3 col-md-6" style="padding: 0;">
	{% include image.html img="/img/microservices/cloud-architectureex.jpg" alt="Single Sign On" title="SSO" %}
</div>
</div>
...
<div class="row" style="margin: 0 auto 2.5rem auto; width: 100%;">
  <div class="col-md-offset-3 col-md-6" style="padding: 0;">
	{% include image.html img="/img/microservices/login.png" alt="Single Sign On" title="SSO" %}
</div>
</div>
...
<div class="row" style="margin: 0 auto 2.5rem auto; width: 100%;">
  <div class="col-md-offset-3 col-md-6" style="padding: 0;">
	{% include image.html img="/img/microservices/jwt-example.png" alt="Single Sign On" title="SSO" %}
</div>
</div>
...
# Creating The User Authentication Authorization Server
...
<div class="row" style="margin: 0 auto 2.5rem auto; width: 100%;">
  <div class="col-md-offset-3 col-md-6" style="padding: 0;">
	{% include image.html img="/img/microservices/SSOSecurity.jpg" alt="Single Sign On" title="SSO" %}
</div>
</div>
...
When we want to secure our data, we want to be sure they are in a secure location and encrypted.
There are various solutions to this problem, if you have your platform running in the cloud, you can look at the in house Key Management Service or you can build your own config server.
Since the 12 factor app teaches us that code and configuration has to be separated, Spring provides us with the Spring Cloud Config Server.
...
# Secure The Inter-service/Inter-process communication
When choosing a communication protocol, the most used are REST and messaging. 
In this post, we will cover REST with Feign as communication client. 
...
Feign gives support by adding an Authorization header inside the REST request before sending it to the other microservice.
Aside of adding this security, Feign has integrated resilience and client-side load balancing inside the client.
Therefore, using the Feign client will prevent cascading failures and provide a fallback mechanism.

## Configuration
To add Feign to the classpath, we will be adding the Feign starter of Spring Cloud. 
...
When creating the Feign client, we have to look at Eureka for the client id.
Behind the scenes, Feign will ask Eureka for the microservice with the proper client id and sets up a channel for communication.
When enabling feign clients on your spring boot application, you can add these interfaces and annotate them as a Feign client.
As you define the channel of your Feign client, there are a few things to take into consideration. 

* The endpoint value must be the same in both the Feign client as in the API endpoint in the targeted service
* Same goes for the return value, request method and the parameters

{% highlight java %}
@FeignClient(value = "account", fallback = UserClientFallBack.class, configuration = FeignConfig.class)
public interface UserClient {
    @RequestMapping(method = RequestMethod.GET, value = "accounts/{uuid}")
    AccountResource findOne(@RequestHeader("Authorization") String auth, @PathVariable("uuid") UUID uuid);
}
{% endhighlight %}

* `value` 
is the identifier of the microservice you want to communicate to. 
If you'd like to know which id your microservice is, check your service discovery(ex. Eureka).
* `fallback`
is the class where the fallback mechanism is implemented when a failure will occur.
* `configuration`
is the class where you can put extra custom configuration to your client.


### Feign with OAuth2 
Since Spring Cloud has a default way of working with RestTemplate, 
there is no out of the box solution for propagating OAuth2 tokens that are being sent from Zuul.
But there is a way of intercepting the Feign client and add the JWT out of the OAuth2 context.
For a more detailed dive in, be sure to check out [OAuth2 Feign Client](https://jmnarloch.wordpress.com/2015/10/14/spring-cloud-feign-oauth2-authentication/)

### Feign with only JWT
When adding the  `@RequestHeader("Authorization")` annotation in your parameters, 
Feign will add this to your request headers and execute the request with the Authorization header.
So how does this JWT travel from the incoming request to the outgoing request? 
This will be covered in the part where we go deep dive in securing the microservice itself. 
